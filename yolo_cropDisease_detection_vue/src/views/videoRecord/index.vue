	...
	<div class="record-page layout-padding">
		<div class="record-panel layout-padding-auto layout-padding-view">
			<section class="record-hero">
				<div>
					<span class="hero-badge">视频识别记录</span>
					<h2>查看上传视频后的识别结果与逐帧病虫害日志</h2>
					<p>支持列表筛选、详情查看和逐帧时间戳日志展开，直接在记录页就能看到每一帧的识别结果。</p>
				</div>
				<div class="hero-side">
					<span>当前账号</span>
			...
			<section class="table-card">
				<div class="table-head">
					<h3>视频识别结果</h3>
					<p>列表中可以直接展开查看逐帧识别日志，也可以进入详情页同步查看视频与时间轴结果。</p>
				</div>
					...
					<el-table-column prop="inputVideo" label="原始视频" width="220" align="center">
						<template #default="scope">
							<div class="video-preview-card">
								<video class="video" preload="metadata" controls :key="scope.row.inputVideo + uniqueKey">
	...
	request
		...
		.then((res) => {
			...
			state.tableData.data = (res.data.records || []).map((item: any, index: number) => ({
				...item,
				num: index + 1,
				inputVideo: resolveFileUrl(item.inputVideo),
...
const show = (row: any) => {
	window.open(
		router.resolve({
			name: 'videoShow',
			query: { id: String(row.id), mode: 'video' },
		}).href
	);
};
